...
#noisy_mat4d = np.load('./oed_experiments/random_sample_zmap/tile7x7/digit2_var1.0/noisy_mat4d.npy') #-- for digit 2 old code


sample_dir = './oed_experiments/oed/tile7x7/digit{}_var{}_N{}/iter{}'.format(PARAMS.digit, noise_var, N, iter_no)
...
with tf.Session(graph=g) as sess:
    ...
    if (iter_no==0):
        ...
        rec_loss = np.linalg.norm(g_z_map[0,:,:,0]-test_sample[:,:,0])/784
    ...
    np.save(sample_dir+'/x_map', g_z_map[0,:,:,0])
    ...
    axs[1][1].set_title('g(z_map) - x | Reconstruction error(per pixel)={}'.format(rec_loss))    
